    ...
    <el-row>
      <el-col>
        <el-form :inline="true" ref="filtersForm" :model="filtersForm">
          ...
          <el-form-item>
            <el-select v-model="filtersForm.code" placeholder="请选择日志内容" style="width: 100%;">
              <el-option
...
<script>
  ...
  let initAllContents = function () {
      this.typeIds = []
      this.$api.post(this.GLOBAL.httpRequestUrl + '/log/getAllNames', null, response => {
          if (response.status == 200) {
              this.contentOptions = response.data.content;
              for(var i = 0; i < this.contentOptions.length; i ++){
                  this.typeIds[i] = this.contentOptions[i].type
              }
          }
      });
  }
